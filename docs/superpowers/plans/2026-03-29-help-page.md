# Hjælpeside Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tilføj en hjælpeside til biSPCharts der forklarer SPC-grundbegreber og app-vejledning, tilgængelig via et link i headeren, adskilt fra wizard-flowet.

**Architecture:** Hjælpesiden implementeres som et Shiny-modul (`mod_help_ui`/`mod_help_server`) med et `nav_panel` i navbar. Indholdet er statisk HTML bygget med Shiny tags. Screenshots placeres som PNG-filer i `inst/app/www/help/`. Modulet har minimal server-logik (ingen reaktivitet nødvendig).

**Tech Stack:** Shiny UI tags (tags$h2, tags$p, tags$img), bslib layout, bslib::nav_spacer for visuelt skel

---

### Task 1: Opret hjælpe-modul UI med al indhold

**Files:**
- Create: `R/mod_help_ui.R`

- [ ] **Step 1: Opret `R/mod_help_ui.R` med komplet indhold**
...
mod_help_ui <- function(id) {
  ...
  shiny::div(
    ...
    shiny::tags$section(
      ...
      shiny::tags$ul(
        shiny::tags$li(shiny::tags$strong("Start altid med et seriediagram"),
          " \u2014 det er det simpleste og mest robuste"),
        shiny::tags$li(shiny::tags$strong("Brug mindst 12\u201315 datapunkter"),
          " for at Anh\u00f8j-reglerne kan detektere signaler p\u00e5lideligt"),
        shiny::tags$li(shiny::tags$strong("Marker skift kun ved kendte proces\u00e6ndringer"),
          " \u2014 ikke ved tilf\u00e6ldig variation"),
        shiny::tags$li(shiny::tags$strong("Lad data tale:"),
          " undg\u00e5 at overfortolke enkelte punkter eller korte perioder"),
        shiny::tags$li(shiny::tags$strong("Vis data som tidsserier,"),
          " ikke som s\u00f8jlediagrammer eller tabeller \u2014 r\u00e6kkef\u00f8lgen er vigtig"),
        shiny::tags$li(shiny::tags$strong("En stabil proces er ikke n\u00f8dvendigvis en god proces"),
    ...
    shiny::tags$section(
      ...
      shiny::tags$ul(
        shiny::tags$li(
          ...
          " ",
          shiny::tags$a(href = "https://anhoej.github.io/spc4hc/",
        ...
        shiny::tags$li(
          ...
          " ",
          shiny::tags$a(href = "https://anhoej.github.io/qicharts2/articles/qicharts2.html",
        ...
        shiny::tags$li(
          ...
          " 2015. ",
          shiny::tags$a(href = "https://qualitysafety.bmj.com/content/26/1/81",
        ...
        shiny::tags$li(
          ...
          " ",
          shiny::tags$a(href = "https://www.anhoej.net/jacob_fag_spc-manifest.html",
        ...
        shiny::tags$li(
          ...
          " ",
          shiny::tags$a(href = "https://www.anhoej.net/jacob_fag_det_begyndte_med_oel.html",
...
- [ ] **Step 2: Verificer at R kan parse filen**

Run: `TMPDIR=/tmp R --no-save -e "parse('R/mod_help_ui.R'); cat('OK\n')"`
Expected: `OK`
...
git commit -m "feat(help): opret hjælpeside UI modul med SPC-teori og app-vejledning"
```

---

### Task 2: Opret hjælpe-modul server (minimal)

**Files:**
- Create: `R/mod_help_server.R`

- [ ] **Step 1: Opret `R/mod_help_server.R`**
...
```bash
git add R/mod_help_server.R
git commit -m "feat(help): opret minimal hjælpe-modul server"
```

---

### Task 3: Tilføj hjælpe-panel til navbar

**Files:**
- Modify: `R/app_ui.R:50-57`
- Modify: `R/app_server_main.R:213`

- [ ] **Step 1: Tilføj nav_spacer og hjælpe-panel i `R/app_ui.R`**

I `R/app_ui.R`, efter Eksportér nav_panel (linje 55) og før den lukkende parentes af `page_navbar` (linje 57), tilføj:
...
- [ ] **Step 2: Initialiser help modul i `R/app_server_main.R`**

I `R/app_server_main.R`, efter eksport-modul initialisering (linje 213), tilføj:
...
- [ ] **Step 3: Verificer at begge filer parser**

Run: `TMPDIR=/tmp R --no-save -e "parse('R/app_ui.R'); parse('R/app_server_main.R'); cat('OK\n')"`
Expected: `OK`
...
git commit -m "feat(help): tilføj hjælpe-panel i navbar med nav_spacer"
```

---

### Task 4: Opret screenshot-mappe med placeholder-billeder

**Files:**
- Create: `inst/app/www/help/` directory
- Create: placeholder PNG-filer

- [ ] **Step 1: Opret mappe og placeholder-filer**

```bash
mkdir -p inst/app/www/help
```

Opret en simpel placeholder for hvert billede (så appen ikke viser broken images):

```bash
# Generér minimale placeholder PNG-filer med R
R --no-save -e '
placeholders <- c(
  "01-run-chart-stabil",
  "02-variation-sammenligning",
  "03-diagram-annoteret",
  "04-anhoej-signal",
  "05-p-chart",
  "06a-trin1-upload",
  "06b-trin2-analyser",
...
for (name in placeholders) {
  path <- file.path("inst/app/www/help", paste0(name, ".png"))
  png(path, width = 700, height = 350, bg = "#f8f9fa")
  par(mar = c(0, 0, 0, 0))
  plot.new()
  text(0.5, 0.5, paste("Screenshot:", name), cex = 1.5, col = "#6c757d")
  dev.off()
}
cat("Created", length(placeholders), "placeholders\n")
'
```
...
```bash
git add inst/app/www/help/
git commit -m "feat(help): opret screenshot-mappe med placeholder-billeder"
```

---

### Task 5: Commit spec og plan

**Files:**
- Add: `docs/superpowers/specs/2026-03-29-help-page-design.md`
- Add: `docs/superpowers/plans/2026-03-29-help-page.md`

- [ ] **Step 1: Commit dokumentation**

```bash
git add docs/superpowers/specs/2026-03-29-help-page-design.md docs/superpowers/plans/2026-03-29-help-page.md
git commit -m "docs: tilføj design spec og plan for hjælpeside"
```

---

## Screenshot-guide (manuelt trin efter implementation)

Når koden er deployed, tag disse screenshots fra appen og erstat placeholder-billederne:

| Fil | Hvad der vises | Forberedelse |
|-----|---------------|-------------|
| `01-run-chart-stabil.png` | Run chart uden signaler | Upload eksempeldata, trin 2, run chart |
| `02-variation-sammenligning.png` | To diagrammer side om side (stabil + skift) | Tag to separate screenshots og sammensæt |
| `03-diagram-annoteret.png` | Diagram med pile/labels | Tag screenshot af run chart, tilføj annotationer i billedbehandling |
| `04-anhoej-signal.png` | Diagram med signal + value boxes | Trin 2, P-chart med eksempeldata (36 signaler), inkluder value boxes |
| `05-p-chart.png` | P-chart med kontrolgrænser | Trin 2, P-chart med eksempeldata |
| `06a-trin1-upload.png` | Upload-siden efter fil-upload | Trin 1 efter upload af eksempeldata |
| `06b-trin2-analyser.png` | Analyse-siden med diagram | Trin 2 med fuldt diagram |
| `06c-trin3-eksporter.png` | Eksport-siden med PDF preview | Trin 3 med PDF preview synlig |
