...
Detektion og UI-flow for upload af Excel-filer (`.xlsx`, `.xls`) i biSPCharts.
Dækker tre scenarier: biSPCharts gem-format (auto-restore), single-sheet
(auto-indlæs), og standard multi-sheet (eksplicit sheet-picker). Sikrer
at brugere kan vælge hvilket faneblad data skal hentes fra, mens
gem-format-restore og simpel single-sheet-flow forbliver friktionsfri.
