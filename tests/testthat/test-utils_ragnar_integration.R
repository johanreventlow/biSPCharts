...
test_that("load_ragnar_store returns NULL if ragnar package not installed", {
  ...
  with_mock(
    `base::requireNamespace` = function(package, quietly = TRUE) FALSE,
    {
...
test_that("load_ragnar_store returns NULL if store directory missing", {
  ...
  with_mock(
    `base::requireNamespace` = function(package, quietly = TRUE) TRUE,
    `base::system.file` = function(...) "",
    {
      store <- load_ragnar_store()
      expect_null(store)
...
test_that("reset_ragnar_store_cache clears cache and load attempt flag", {
  ...
  with_mock(
    `base::requireNamespace` = function(package, quietly = TRUE) TRUE,
    `base::system.file` = function(...) "/fake/path",
    `base::dir.exists` = function(path) TRUE,
    `ragnar::ragnar_store` = mock_ragnar_store,
    {
      # Load store
      store1 <- load_ragnar_store()
...
test_that("query_spc_knowledge constructs query from SPC metadata", {
  ...
  with_mock(
    ...
    `base::dir.exists` = function(path) TRUE,
    `ragnar::ragnar_store` = mock_ragnar_store,
    `ragnar::ragnar_retrieve` = function(store, query, n, method) {
...
test_that("query_spc_knowledge handles NULL signals gracefully", {
  ...
  with_mock(
    ...
    `base::dir.exists` = function(path) TRUE,
    `ragnar::ragnar_store` = mock_ragnar_store,
    `ragnar::ragnar_retrieve` = mock_ragnar_retrieve,
    {
      context <- query_spc_knowledge(
        chart_type = "run",
        signals = NULL,
...
test_that("get_rag_config returns default config when golem config missing", {
  with_mock(
    `golem::get_golem_options` = function(key) NULL,
    {
...
test_that("get_rag_config loads config from golem options", {
  with_mock(
    `golem::get_golem_options` = function(key) {
      if (key == "ai") {
        list(
          rag = list(
            enabled = FALSE,
